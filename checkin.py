...
def mask_sensitive_info(text, mask_type='name'):
	"""屏蔽敏感信息

	Args:
		text: 要屏蔽的文本
		mask_type: 屏蔽类型，'name' 或 'amount'
	"""
	if mask_type == 'amount':
		return '***'

	# 对于账号名称，保留前2个字符，其余用*替代
...
async def check_in_with_playwright(account_name: str, user_cookies: dict, api_user: str):
	...
	masked_name = mask_sensitive_info(account_name, 'name')
				...
				if user_info_result.get('success'):
					try:
						...
						if data.get('success'):
							...
							masked_quota = mask_sensitive_info(str(quota), 'amount')
							masked_used = mask_sensitive_info(str(used_quota), 'amount')
							user_info = {
								'success': True,
								'quota': quota,
								'used_quota': used_quota,
								'display': f':money: Current balance: ${masked_quota}, Used: ${masked_used}'
							}
	...
	if balance_changed:
		for i, account in enumerate(accounts):
			...
			if account_key in current_balances:
				account_name = get_account_display_name(account, i)
				masked_name = mask_sensitive_info(account_name, 'name')
				masked_quota = mask_sensitive_info(str(current_balances[account_key]["quota"]), 'amount')
				masked_used = mask_sensitive_info(str(current_balances[account_key]["used"]), 'amount')
				# 只添加成功获取余额的账号，且避免重复添加
				account_result = f'[BALANCE] {masked_name}'
				account_result += f'\n:money: Current balance: ${masked_quota}, Used: ${masked_used}'
				# 检查是否已经在通知内容中（避免重复）
